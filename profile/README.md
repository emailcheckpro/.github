...
Submit one email address and get its deliverability result in the same HTTP response. Works on any domain, from Gmail to a company's own. Up to 100 addresses per synchronous request. Whole lists go through the asynchronous bulk API instead.
...
| **[Email bulk tasks](https://github.com/emailcheckpro/email-bulk-checker-api)** | Bulk (async) | 2 products | OpenAPI contract, `product.json`, `llms.txt`, examples in 7 languages |
